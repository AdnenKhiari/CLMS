...
const getAllBooks = async (filter = {})=> new Promise((resolve,reject)=>{
    ...
    const callBack = (err,result,info)=>{
        ...
        if(!result || result.length === 0){
            return reject(new ApiError("Resource Not Found","Book with that ID Or Specs Not Found",StatusCodes.UNPROCESSABLE_ENTITY))
        }
    ...
    if(values.ISBN){
        ...
        mysqlClient.query(STATEMENT,values.ISBN,(err,result,info)=>{
            if(err)
                return reject(err)
            if(!result || result.length === 0){
                return reject(new ApiError("Resource Not Found","ISBN Not Found",StatusCodes.UNPROCESSABLE_ENTITY))
            }
...
const deleteABook = async (filters) => new Promise((resolve,reject)=>{
    ...
    mysqlClient.query(STATEMENT_SELECT,[filters.ID],(err,result,info)=>{
        ...
        if(result.length === 0 || !result)
            return reject(new ApiError("Resource Not Found","Book with that ID Not Found",StatusCodes.UNPROCESSABLE_ENTITY))
        const STATEMENT = 'DELETE FROM Books WHERE ID=?'
        mysqlClient.query(STATEMENT,[filters.ID],(err2,result2,info2)=>{
            if(err2)
                return reject(err2)
            if(!result2.affectedRows === 0)
                return reject(new ApiError("Resource could not be updated","Book with that ID was Found but could not be updated",StatusCodes.INTERNAL_SERVER_ERROR))
            return resolve(format_result(result))
        })
    })
...
const patchABook = async (values) => new Promise((resolve,reject)=>{
        ...
        mysqlClient.query(SELECT_STATEMENT,[values.ID],(err,result,info)=>{
            if(err)
                return reject(err)
            if(result && result.length === 0)
                return reject(new ApiError("Resource Not Found","Book Not Found using that ID ",StatusCodes.UNPROCESSABLE_ENTITY))
        ...
        mysqlClient.query(UPDATE_STATEMENT,[values.title,values.publisher,values.author,values.ISBN,values.publication_date,values.cover_url,values.ID],(error,result,info)=>{
            ...
            if(result.affectedRows === 0)
                return reject(new ApiError("Resource could not be update","Book was Found but could not be updated",StatusCodes.INTERNAL_SERVER_ERROR))
            return resolve(format_result([entries,values]))
        })
}))
